...
def compute_class_weights(train_loader, num_classes):
    ...
    for sampled_batch in tqdm(train_loader, desc="Computing weights"):
        label_batch = sampled_batch['label']
        for c in range(num_classes):
            class_counts[c] += (label_batch == c).sum().item()
    ...
    # # Clip to prevent training instability (max ratio 100:1)
    # weights = torch.clamp(weights, min=weights.min(), max=weights.min() * 100)
    
    print(f"Computed class weights: {weights}")
    print(f"Weight ratios: {weights / weights.min()}")
...
def trainer_synapse(args, model, snapshot_path):
    ...
    from datasets.dataset_zebrafish import Zebrafish_dataset, RandomGenerator
    ...
    #                          transform=transforms.Compose(
    #                              [RandomGenerator(output_size=[args.img_size, args.img_size])]))
    db_train = Zebrafish_dataset(base_dir=args.root_path, mode="train",
                               transform=transforms.Compose(
                                   [RandomGenerator(output_size=[args.img_size, args.img_size])]))
    
    db_val = Zebrafish_dataset(base_dir=args.root_path, mode="val",
                             transform=transforms.Compose(
                                 [RandomGenerator(output_size=[args.img_size, args.img_size])]))
...
def trainer_zebrafish(args, model, snapshot_path):
    from datasets.dataset_zebrafish import Zebrafish_dataset, RandomGenerator
    #logging.basicConfig(filename=snapshot_path + "/log.txt", level=logging.INFO,
                        #format='[%(asctime)s.%(msecs)03d] %(message)s', datefmt='%H:%M:%S')
    logging.basicConfig(filename=snapshot_path + "/log_dice_dominant.txt", level=logging.INFO,
                        format='[%(asctime)s.%(msecs)03d] %(message)s', datefmt='%H:%M:%S')
    #logging.basicConfig(filename=snapshot_path + "/log_loss_equal.txt", level=logging.INFO,
                        #format='[%(asctime)s.%(msecs)03d] %(message)s', datefmt='%H:%M:%S')
    logging.getLogger().addHandler(logging.StreamHandler(sys.stdout))
    logging.info(str(args))
    base_lr = args.base_lr
    ...
    db_train = Zebrafish_dataset(base_dir=args.root_path, mode="train",
                               transform_augment = augmentation_transforms,transform=transforms.Compose(
                                   [RandomGenerator(output_size=[args.img_size, args.img_size])]))
    db_val = Zebrafish_dataset(base_dir=args.root_path, mode="val",transform_augment = augmentation_transforms,
                             transform=transforms.Compose(
                                 [RandomGenerator(output_size=[args.img_size, args.img_size])]))
    print("The length of train set is: {}".format(len(db_train)))
    ...
    for epoch_num in iterator:
        ...
        for i_batch, sampled_batch in tqdm(enumerate(train_loader), desc=f"Train: {epoch_num}", total=len(train_loader),
            ...
            outputs = F.interpolate(outputs, size=label_batch.shape[-2:], mode='bicubic', align_corners=False)
            loss_ce = ce_loss(outputs, label_batch[:].long())
            loss_dice = dice_loss(outputs, label_batch, softmax=True)
            loss = 0.25 * loss_ce + 0.75 * loss_dice
            #loss = 0.4 * loss_ce + 0.6 * loss_dice
        ...
        batch_ce_loss /= len(train_loader)
        batch_dice_loss /= len(train_loader)
        batch_loss = 0.25 * batch_ce_loss + 0.75 * batch_dice_loss
        #batch_loss = 0.4 * batch_ce_loss + 0.6 * batch_dice_loss
        #batch_loss = 0.5 * batch_ce_loss + 0.5 * batch_dice_loss
        logging.info('Train epoch: %d : loss : %f, loss_ce: %f, loss_dice: %f' % (
            epoch_num, batch_loss, batch_ce_loss, batch_dice_loss))
        ...
        if (epoch_num + 1) % args.eval_interval == 0:
            ...
            with torch.no_grad():
                ...
                val_losses.append((epoch_num, batch_loss))
                
                if batch_loss < best_loss:
                    #save_mode_path = os.path.join(snapshot_path, 'best_model.pth')
                    save_mode_path = os.path.join(snapshot_path, 'best_model_dice_dominant.pth')
                    #save_mode_path = os.path.join(snapshot_path, 'best_model_loss_equal.pth')
                    torch.save(model.state_dict(), save_mode_path)
                    best_loss = batch_loss
                else:
                    #save_mode_path = os.path.join(snapshot_path, 'last_model.pth')
                    save_mode_path = os.path.join(snapshot_path, 'last_model_dice_dominant.pth')
                    #save_mode_path = os.path.join(snapshot_path, 'last_model_loss_equal.pth')
                    torch.save(model.state_dict(), save_mode_path)
                logging.info("save model to {}".format(save_mode_path))
    ...
    if val_losses:
        ...
        ax.plot(val_epochs, val_vals,
                label='Val loss', color='coral', linewidth=2,
                linestyle='--', marker='o', markersize=4)
    ax.set_xlabel('Epoch')
    ax.set_ylabel('Loss  (0.25·CE + 0.75·Dice)')
    ax.set_title('Training curve — saturation detection')
    ax.legend()
    ax.grid(True, alpha=0.3)
    fig.tight_layout()
    plot_path = os.path.join(snapshot_path, 'loss_curve_dice_dominant.png')
